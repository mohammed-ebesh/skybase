...
import { formSchema } from "@/app/schema/officerSchema";
import {
  officerRanks,
  religions,
  bloodTypes,
...
function MilitaryForm() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!parseResult.success) {
      ...
      // يمكنك رفع الصورة إلى السيرفر هنا أو حفظها في قاعدة بيانات...
    }
  };
  ...
  return (
    <div className="max-w-screen-lg mx-auto p-4">
      <h1 className="text-2xl font-bold text-center mb-6">إضافة كرت ضابط</h1>
      <form onSubmit={handleSubmit} className="grid grid-cols-3 gap-4">
          ...
          {imagePreview && (
            <div className="col-span-2 flex items-center">
              <div className="text-gray-700">
                <p className="font-semibold">تم اختيار الصورة:</p>
                <p className="text-sm">اضغط على الصورة لاختيار صورة أخرى</p>
              </div>
            </div>
          )}
        </div>
        ...
        {renderInput(
          "تاريخ الترقّي للرتبة الحالية:",
          "currentRankPromotionDate",
          "date"
        )}
        {renderInput(
          "تاريخ الترقّي للرتبة القادمة:",
          "nextRankPromotionDate",
          "date"
        )}
        ...
        {renderInput("الفئة (محارب/مهندس/فني...):", "category")}
        ...
        {renderInput("عدد الأولاد:", "numberOfChildren", "number", { min: 0 })}
        ...
        {renderInput("الطول (سم):", "height", "number", { min: 0 })}
        {renderInput("الوزن (كجم):", "weight", "number", { min: 0 })}
